...
class DenseBlock(nn.Module):
    def __init__(self, num_layers, inplances, growth_rate, bn_size, drop_rate=0):
        ...
        for i in range(num_layers):
            layers.append(
                _DenseLayer(
                    inplances + i * growth_rate, growth_rate, bn_size, drop_rate
                )
            )
...
class DenseNet(nn.Module):
    def __init__(
        self, init_channels=64, growth_rate=32, blocks=[6, 12, 24, 16], num_classes=10
    ):
        ...
        self.layer1 = DenseBlock(
            num_layers=blocks[0],
            inplances=num_features,
        ...
        self.transition1 = _TransitionLayer(
            inplace=num_features, plance=num_features // 2
        )
        ...
        self.transition2 = _TransitionLayer(
            inplace=num_features, plance=num_features // 2
        )
        ...
        self.transition3 = _TransitionLayer(
            inplace=num_features, plance=num_features // 2
        )
